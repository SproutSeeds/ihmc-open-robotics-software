/*
 * This class was automatically generated by jros2.
 * Do not modify this file directly.
 */
package behavior_msgs.msg.dds;

import us.ihmc.fastddsjava.cdr.CDRBuffer;
import us.ihmc.fastddsjava.cdr.idl.*;
import us.ihmc.jros2.ROS2Message;

/**
<p>Source (behavior_msgs/AI2RNodeStateMessage):
<pre>{@code
# Parent state fields
behavior_msgs/BehaviorTreeNodeStateMessage state

# Definition
behavior_msgs/AI2RNodeDefinitionMessage definition

}</pre>
*/
public class AI2RNodeStateMessage implements ROS2Message<AI2RNodeStateMessage>
{
   public static final java.lang.String name = "behavior_msgs::msg::dds_::AI2RNodeStateMessage_";

   /**
      Parent state fields
   */
   private final behavior_msgs.msg.dds.BehaviorTreeNodeStateMessage state_;
   /**
      Definition
   */
   private final behavior_msgs.msg.dds.AI2RNodeDefinitionMessage definition_;

   public AI2RNodeStateMessage()
   {
      state_ = new behavior_msgs.msg.dds.BehaviorTreeNodeStateMessage();
      definition_ = new behavior_msgs.msg.dds.AI2RNodeDefinitionMessage();

   }

   @Override
   public int calculateSizeBytes(int currentAlignment)
   {
      int initialAlignment = currentAlignment;

      currentAlignment += state_.calculateSizeBytes(currentAlignment);
      currentAlignment += definition_.calculateSizeBytes(currentAlignment);

      return currentAlignment - initialAlignment;
   }

   @Override
   public void serialize(CDRBuffer buffer)
   {
      state_.serialize(buffer);
      definition_.serialize(buffer);

   }

   @Override
   public void deserialize(CDRBuffer buffer)
   {
      state_.deserialize(buffer);
      definition_.deserialize(buffer);

   }

   @Override
   public void set(AI2RNodeStateMessage from)
   {
      state_.set(from.state_);
      definition_.set(from.definition_);

   }

   public behavior_msgs.msg.dds.BehaviorTreeNodeStateMessage getState()
   {
      return state_;
   }

   public behavior_msgs.msg.dds.AI2RNodeDefinitionMessage getDefinition()
   {
      return definition_;
   }


}
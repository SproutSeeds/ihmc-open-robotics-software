/*
 * This class was automatically generated by jros2.
 * Do not modify this file directly.
 */
package controller_msgs.msg.dds;

import us.ihmc.fastddsjava.cdr.CDRBuffer;
import us.ihmc.fastddsjava.cdr.idl.*;
import us.ihmc.jros2.ROS2Message;

/**
This message is part of the IHMC whole-body controller API.
This message commands the controller to apply a wrench (force & moment) profile on a hand given a list of trajectory points.
A linear interpolation is used to interpolate between trajectory points.
<p>Source (controller_msgs/HandWrenchTrajectoryMessage):
<pre>{@code
# This message is part of the IHMC whole-body controller API.
# This message commands the controller to apply a wrench (force & moment) profile on a hand given a list of trajectory points.
# A linear interpolation is used to interpolate between trajectory points.

byte ROBOT_SIDE_LEFT = 0
byte ROBOT_SIDE_RIGHT = 1

# Unique ID used to identify this message, should preferably be consecutively increasing.
uint64 sequence_id

# When the robot is walking, restrictions on upper-body motion may be applied.
# To by-pass the safety check and force the execution of this message, set this field to true.
bool force_execution false

# Specifies which hand will execute the trajectory.
byte robot_side 255
# The trajectory information for the force/moment to be achieved by the end-effector.
WrenchTrajectoryMessage wrench_trajectory}</pre>
*/
public class HandWrenchTrajectoryMessage implements ROS2Message<HandWrenchTrajectoryMessage>
{
   public static final java.lang.String name = "controller_msgs::msg::dds_::HandWrenchTrajectoryMessage_";

   public static final byte ROBOT_SIDE_LEFT = 0;
   public static final byte ROBOT_SIDE_RIGHT = 1;
   /**
      Unique ID used to identify this message, should preferably be consecutively increasing.
   */
   private long sequence_id_;
   /**
      When the robot is walking, restrictions on upper-body motion may be applied.
      To by-pass the safety check and force the execution of this message, set this field to true.
   */
   private boolean force_execution_;
   /**
      Specifies which hand will execute the trajectory.
   */
   private byte robot_side_;
   /**
      The trajectory information for the force/moment to be achieved by the end-effector.
   */
   private final controller_msgs.msg.dds.WrenchTrajectoryMessage wrench_trajectory_;

   public HandWrenchTrajectoryMessage()
   {
      force_execution_ = (boolean) false;
      robot_side_ = (byte) 255;
      wrench_trajectory_ = new controller_msgs.msg.dds.WrenchTrajectoryMessage();

   }

   @Override
   public int calculateSizeBytes(int currentAlignment)
   {
      int initialAlignment = currentAlignment;

      currentAlignment += 8 + CDRBuffer.alignment(currentAlignment, 8); // sequence_id_
      currentAlignment += 1 + CDRBuffer.alignment(currentAlignment, 1); // force_execution_
      currentAlignment += 1 + CDRBuffer.alignment(currentAlignment, 1); // robot_side_
      currentAlignment += wrench_trajectory_.calculateSizeBytes(currentAlignment);

      return currentAlignment - initialAlignment;
   }

   @Override
   public void serialize(CDRBuffer buffer)
   {
      buffer.writeLong(sequence_id_);
      buffer.writeBoolean(force_execution_);
      buffer.writeByte(robot_side_);
      wrench_trajectory_.serialize(buffer);

   }

   @Override
   public void deserialize(CDRBuffer buffer)
   {
      sequence_id_ = buffer.readLong();
      force_execution_ = buffer.readBoolean();
      robot_side_ = buffer.readByte();
      wrench_trajectory_.deserialize(buffer);

   }

   @Override
   public void set(HandWrenchTrajectoryMessage from)
   {
      sequence_id_ = from.sequence_id_;
      force_execution_ = from.force_execution_;
      robot_side_ = from.robot_side_;
      wrench_trajectory_.set(from.wrench_trajectory_);

   }

   public long getSequenceId()
   {
      return sequence_id_;
   }

   public void setSequenceId(long sequence_id_)
   {
      this.sequence_id_ = sequence_id_;
   }

   public boolean getForceExecution()
   {
      return force_execution_;
   }

   public void setForceExecution(boolean force_execution_)
   {
      this.force_execution_ = force_execution_;
   }

   public byte getRobotSide()
   {
      return robot_side_;
   }

   public void setRobotSide(byte robot_side_)
   {
      this.robot_side_ = robot_side_;
   }

   public controller_msgs.msg.dds.WrenchTrajectoryMessage getWrenchTrajectory()
   {
      return wrench_trajectory_;
   }


}